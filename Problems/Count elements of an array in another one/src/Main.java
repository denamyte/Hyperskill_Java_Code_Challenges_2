...
public class Main {
    ...
    public static int[] countOccurrences(int[] first, int[] second) {
        List<Integer> secondList = IntStream.of(second).boxed().collect(Collectors.toList());
        return IntStream.of(first).map(v1 -> (int) secondList.stream().filter(v2 -> v2 == v1).count()).toArray();
    }
