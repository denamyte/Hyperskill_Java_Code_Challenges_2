...
public class Main {
    ...
    private static final Function<LongStream, LongStream> INVERTER = s -> s.isParallel()
            ? s.sequential() : s.parallel();

    private static List<LongStream> invertedStreams(List<LongStream> streams) {
        return streams.stream().map(INVERTER).collect(Collectors.toList());
    }
    ...
    public static void main(String[] args) {
        ...
        List<LongStream> streams = Stream
                .iterate(0, i -> i < parallelFlags.size(), i -> i + 1)
                .map(i -> {
